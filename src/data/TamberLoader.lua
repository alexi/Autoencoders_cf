local tamberLoader, parent = torch.class('cfn.tamberLoader', 'cfn.DataLoader')

-- Loads tamber-generated ratings
function tamberLoader:LoadRatings(conf)
  ...
  if conf.split == "time" then
    ...
    for line in ratesfile:lines() do
      size = size + 1
      local userIdStr, itemIdStr, ratingStr, timeStr = line:match('(%d+)::(%d+)::(%d%.?%d?)::(%d+)')
   ...
   -- Step 1 : Retrieve movies'scores...th
    for line in ratesfile:lines() do
      ...
      local userIdStr, itemIdStr, ratingStr = line:match('(%d+)::(%d+)::(%d%.?%d?)::(%d+)')
...
function tamberLoader:LoadMetaU(conf) 
...
function tamberLoader:LoadMetaV(conf) 
